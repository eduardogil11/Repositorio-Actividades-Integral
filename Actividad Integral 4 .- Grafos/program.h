...
class Heap {
    private:
	    ...
	    unsigned int tam;
	    unsigned int cont;
      int superMario, pokémon, theLegendofZelda, kirby, donkeyKong,fireEmblem;	
...
void Heap<T>::add_data(vector<string> &s) {
	...
	push(superMario), push(pokémon), push(theLegendofZelda), push(kirby), push(donkeyKong), push(fireEmblem);
}
