...
class Heap {
    private:
	    ...
	    unsigned int tam;
	    unsigned int cont;
      int superMario, fireEmblem, theLegendofZelda, kirby, donkeyKong,pokemon;	
...
void Heap<T>::add_data(vector<string> &s) {
	...
	push(superMario), push(fireEmblem), push(theLegendofZelda), push(kirby), push(donkeyKong), push(pokemon);
}
